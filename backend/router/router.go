...
func Setup(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	exportHandler := &handlers.ExportHandler{DB: db, Audit: audit}
	...
	userAPI := r.Group("/api/v1")
	{
		...
		userProtected.Use(middleware.AuthMiddleware(db, cfg))
		{
			...
			// Marketplace (browse / buy / favorite deposits) + purchases
			...
			// Collecte code-barres (pro/artisan)
			...
			userProtected.GET("/provider/prestations/:id", userProviderHandler.ShowPrestation)
			userProtected.GET("/provider/prestations/:id/participants", userProviderHandler.ListParticipants)
			userProtected.POST("/provider/prestations/:id/images", userProviderHandler.UploadPrestationImage)
			userProtected.DELETE("/provider/prestations/:id/images/:imageId", userProviderHandler.DeletePrestationImage)
	...
	api := r.Group("/api/admin/v1")
	{
		...
		protected.Use(middleware.IsAdmin())
		{
			...
			protected.GET("/export/datasets", exportHandler.Datasets)
